...
public class ClassMappingController {
    ...
    public String modifyAndDelete(Model model) {
        model.addAttribute("message", "POST 방식의 수정, 삭제 둘 다 처리하는 핸들러 메소드 호출됨...");
        return "mappingResult";
    }

    // 3. Path variable (url 경로를 타고 오는 값)
    /* 필기.
    *   @PathVariable 어노테이션을 통해 요청 URL 로부터 변수를 받아올 수 있다.
    *   path variable 로 전달되는 {변수명] 값은 반드시 매개변수 명과 일치해야 한다.
    *   만약 동일하지 않으면 @PathVariable("이름")을 설정해야 한다.
    * */
    @GetMapping("/detail/{orderNo}")
    public String detail(Model model, @PathVariable int orderNo) {
