...
public class ProblemEXTests {
    ...
    *   독립적인 실행이 가능하며, 기능을 테스트하는 것에 초점이 있다.
    *   또한, 접근 제한자는 사용하지 않아도 되지만, private 는 안된다.
    * */
    ...
    * */
    ...
    void testDirectSQL() throws SQLException {
        ...
        *   Assertions 클래스는 Test 를 위한 검증할 수 있는 메소드를 제공해준다.
        * */

//        Assertions.assertNull(menuList);        // 데이터가 있기에 오류 발생
        Assertions.assertNotNull(menuList);     // 정상 동작
    ...
    *   EX) 메뉴 이름에서 메뉴 가격만 조회하는 걸로 수정해주세요~ or 컬럼 추가해주세요
    ...
    *  */
    ...
    /* comment.
    *    SQL 관점에서의 1:N -> 엔티티 간의 연관성을 FK 로 가진다.
    *   JAVA 관점에서의 1:N -> 연관성을 자신의 멤버(필드)로 가진다.
    * */
    ...
    void testEquals() throws SQLException {
        ...
        String query = "SELECT MENU_CODE, MENU_NAME FROM TBL_MENU WHERE MENU_CODE = 4";
        ...
        *   각기 다른 객체에서 담았을 때 생기는 동일성 보장 실패
        *   -> 수정 시 다른 곳에서는 파악 불가능!!!
        *  */
