...
public class OneToManyTest {
    ...
    *   ex) 10 카테고리를 갖는 menu 가 10개면 select 를 그만큼 해야한다.
    *   1개의 엔티티가 여러 개의 엔티티를 로드해야하는 상황은 성능상의 이슈가 발생할 수 있다.
    *   따라서 일단 카테고리만 조회 후 필요 시에 로드를 하게 된다.
    *   @OneToMany(fetch = FetchType.LAZY)
    *   지연 로딩이라고 하는 것인데, 카테고리 조회 시 연관된 메뉴들은 부르기 전까지 조회하지 않고 부를 때 로딩을 하는 것이다.
    *   @ManyToOne(fetch = FetchType.EAGER)
    *   1개의 엔티티를 조회할 때 1개만 조회하면 되므로 즉시로딩(이른 로딩)이 디폴트 값이다.
    *  */
