...
public class ManyToOneTests {
    ...
    * */
    ...
    /* comment.
    *   ManyToOne 은 다수의 엔티티(메뉴)가 하나의 엔티티(카테고리)를 참조하는 상황에서 사용이 된다.
    *   예를 들자면 하나의 카테고리가 여러 개의 메뉴를 가질 수 있는 상황에서 메뉴가 카테고리를 참조하는 것이다.
    *   따라서 이 때 Menu 가 Many, Category 가 One 이 된다.
    * */

    /* comment.
    *   이렇게 연관 관계 형성된 엔티티를 조회하는 방법으로는 다음과 같다.
    *   1. 객체 그래프 탐색(객체 연관 관계를 사용한 조회)
    *   2. 객체 지향 쿼리(JPQL) 사용
    * */

    @DisplayName("N:1 관계의 객체 그래프 탐색을 위한 조회")
    @Test
    void manyToOneFind() {
        // 테스트용 값
        int menuCode = 10;
    ...
    @DisplayName("N:1 관계의 객체 지향 쿼리 이용 테스트")
    @Test
    void manyToOneJPQL() {
    ...
    /* comment.
    *   commit() 을 하게 될 경우 엔티티 객체를 insert 하는 쿼리문이 동작하게 된다.
    *   단, 여기서 고려할 점이 영속성 전이이다.
    *   메뉴 엔티티를 insert 시 PERSIST 속성으로 인해 카테고리 엔티티도 insert 를 시켜야 한다는 의미이다.
    * */
    ...
    void manyToOneInsert(int menuCode, String menuName, int menuPrice, int categoryCode, String categoryName, String orderableStatus) {
        MenuDTO menuInfo = new MenuDTO(
                menuCode, menuName, menuPrice, new CategoryDTO(categoryCode, categoryName, null), orderableStatus
        );
